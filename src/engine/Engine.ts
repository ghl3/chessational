...
import { MoveAndEvaluation, EvaluatedPosition } from "../chess/EvaluatedPosition";
...
export class Engine {
    ...
    _handleEvent = (e: MessageEvent<any>) => {
        ...
        if (parsed.type === 'BESTMOVE' || parsed.type === 'ISMATE' || parsed.type === 'NOMOVE') {
            ...
            if (parsed.type === 'BESTMOVE') {
                this._log('Engine -- Got Best Move: ' + parsed.from + '->' + parsed.to + ' for ' + fen);
            }
            ...
            const positionEvaluation: EvaluatedPosition = { fen, color, best_moves: bestMovesDecorated };
    ...
    static getDeepestEvaluations = (movesAndEvaluations: MoveAndEvaluation[]): MoveAndEvaluation[] => {
    ...
    static selectBestMoves = (color: Color, movesAndEvaluations: MoveAndEvaluation[]): MoveAndEvaluation[] => {
        ...
        const sortedMovesAndEvaluations = Engine.getDeepestEvaluations(movesAndEvaluations).sort(makeComparator((x) => [

            // Sort by "mate in x" for the player, where lower x is better.
            x?.evaluation?.forced_mate?.for === 'PLAYER' ? x?.evaluation?.forced_mate?.in : Number.MAX_VALUE,
            ...
            x?.evaluation?.forced_mate?.for === 'OPPONENT' ? -1 * x?.evaluation?.forced_mate?.in : Number.MAX_VALUE
        ]));
    ...
    static buildMoveAndEvaluationFromInfo = (info: InfoMessage, color: Color): MoveAndEvaluation | null => {
        if ('pv' in info && info['pv'].length > 0) {
            ...
            return { move: move, evaluation: Engine.buildEvaluationFromInfo(info, color) };
        } else {
            return null;
        }
    }
    ...
    // parsed: A dict 
    static buildEvaluationFromInfo = (info: InfoMessage, color: Color): Evaluation => {
        ...
        if (info?.score?.cp != null) {
            ...
            return { forced_mate: { in: mate_in, for: mate_for }, depth: info?.depth };
        } else {
