...
export type BestMoveMessage = { type: 'BESTMOVE', from: string, to: string, promotion?: string };
export type NoMoveMessage = { type: 'NOMOVE' };
export type IsMateMessage = { type: "ISMATE" };
export type UnknownMessage = { type: 'UNKNOWN' };
export type ParsedMessage = ReadyMessage | EngineLoadedMessage | InfoMessage |
    BestMoveMessage | NoMoveMessage | IsMateMessage | UnknownMessage;
...
export class EventParser {
    ...
    static parse = (msg: Message): ParsedMessage => {
        ...
        if (bestmove) {
            if (bestmove[3] !== undefined) {
                return { type: 'BESTMOVE', from: bestmove[1], to: bestmove[2], promotion: bestmove[3] }
            } else {
    ...
    static parseInfo = (info_log: string): InfoMessage => {
        ...
        const keywords = [
            ...
            'score', 'currmove', 'currmovenumber', 'bmc', 'hashfull',
            'nps', 'tbhits', 'sbhits', 'cpuload', 'string', 'refutation',
            'currline'
        ];

        const tokens = info_log.trim().replaceAll("'", "").replaceAll('"', '').split(/\s+/);
        ...
        for (const token of tokens) {
            if (currently_processing != null) {
                ...
                if (!keywords.includes(token)) {
                    ...
                    info_map[currently_processing] = EventParser.handleInfo(currently_processing, current_tokens);
                    current_tokens = [];
